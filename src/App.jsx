...
const CURRENT_SEASON = new Date().getMonth() < 2 ? new Date().getFullYear() - 1 : new Date().getFullYear();
...
function TeamLogo({ id, abbr, size = 28 }) {
  ...
  if (!id || idx >= candidates.length) return <div style={box}>{abbr?.slice(0,3)}</div>;
  return (
    <img
      src={url}
      alt="logo"
      style={{ width: size, height: size, objectFit: "contain", display: "block" }}
      onError={() => { if (idx < candidates.length - 1) setIdx(idx + 1); else setIdx(candidates.length); }}
      onLoad={(e) => { if (!e.currentTarget.naturalWidth && idx < candidates.length - 1) setIdx(idx + 1); }}
    />
  );
}
...
function RoleTag({ role }) {
  ...
  return <span className={cls}><span style={{ fontSize:12, lineHeight:1 }}>{icon}</span>{text}</span>;
}
...
function StatusPill({ text }) {
  ...
  let bg = "#e2e8f0", fg = "#0f172a", bd = "#e2e8f0";
  if (t.includes("final")) { bg = "#ecfdf5"; fg = "#065f46"; bd = "#a7f3d0"; }
  if (t.includes("in progress") || t.includes("delayed")) { bg = "#fffbeb"; fg = "#92400e"; bd = "#fde68a"; }
  if (t.includes("postponed") || t.includes("suspended")) { bg = "#fef2f2"; fg = "#991b1b"; bd = "#fecaca"; }
  return <span className="pill" style={{ background:bg, borderColor:bd, color:fg }}>{text}</span>;
}
...
function statusWithInning(game) {
  ...
  if (game?.isLive && inn) { return `${base} – ${half ? "Top" : "Bot"} ${inn}`; }
  return base;
}
...
function simplifyGame(game) {
  ...
  const homeTeam = home?.team; const awayTeam = away?.team;
  ...
  return {
    ...
    linescore: { currentInning: ls?.currentInning, isTopInning: ls?.isTopInning },
    away: { id: awayTeam?.id, name: awayTeam?.name, abbr: awayTeam?.abbreviation || (awayTeam?.teamName ?? "Away"), record: away?.leagueRecord ? `${away.leagueRecord.wins}-${away.leagueRecord.losses}` : "", score: awayRuns },
    home: { id: homeTeam?.id, name: homeTeam?.name, abbr: homeTeam?.abbreviation || (homeTeam?.teamName ?? "Home"), record: home?.leagueRecord ? `${home.leagueRecord.wins}-${home.leagueRecord.losses}` : "", score: homeRuns },
    isFinal: /final/i.test(game?.status?.detailedState || ""),
    isLive: /in progress|delayed|warmup/i.test(game?.status?.detailedState || ""),
  };
}
...
function ScoresPanel({ date, setDate, tz }) {
  ...
  useEffect(() => {
    function onKey(e){ if (e.key === "ArrowLeft") setDate((d)=>addDays(d,-1)); if (e.key === "ArrowRight") setDate((d)=>addDays(d,1)); }
  ...
  async function fetchScores(forDate) {
    ...
    try {
      ...
      setGames(g);
    } catch(e) { if (e.name !== "AbortError") setError(e.message || "Failed to load scores"); }
    finally { setLoading(false); }
  }
  useEffect(() => { fetchScores(date); }, [date]);
  ...
  return (
    <div style={{ display:"grid", gap:12 }}>
      <div className="controls">
        <button className="btn" onClick={()=>setDate(addDays(date,-1))}>← Prev</button>
        <div className="row">
          <input className="input" type="date" value={date} onChange={(e)=>setDate(e.target.value)} />
          <button className="btn" onClick={()=>setDate(fmtDate(new Date()))}>Today</button>
        </div>
        <button className="btn" onClick={()=>setDate(addDays(date,1))}>Next →</button>
      </div>
      ...
      <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
        ...
        <div style={{ fontSize:12, color:"#64748b" }}>Times in your local zone: {tz}</div>
      </div>
      ...
      {!loading && !error && (filtered.length === 0 ? (
        <div className="card" style={{ padding:24, textAlign:"center" }}>
          ...
          <div style={{ marginTop:8, color:"#334155", fontWeight:600 }}>No MLB games found for {date}.</div>
        ...
        <div style={{ display:"grid", gap:12 }}>
          {filtered.map(g => {
            ...
            return (
              <div key={g.id} className="card two-col" style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:12 }}>
                ...
                <div className="right-info" style={{ padding:12, display:'grid', gap:10, alignContent:'start', borderLeft:'1px solid #e2e8f0' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                    <span className="label">Status:</span>
                    <StatusPill text={statusWithInning(g)} />
                    {extrasBadge(g)}
                    {g.doubleHeader === "Y" && <span className="pill pill-mini">DH G{g.seriesGameNumber}</span>}
                  </div>
                  <div><span className="label">Time:</span> {prettyTime(g.gameDate, tz)}</div>
                  <div><span className="label">Location:</span> {g.venue || "—"}</div>
                </div>
              </div>
            );
          })}
...
function StandingsPanel({ season = CURRENT_SEASON }) {
  ...
  const numCell = { padding:8, textAlign:"right", fontVariantNumeric:"tabular-nums", fontFeatureSettings:"'tnum' 1, 'lnum' 1", whiteSpace:"nowrap" };
      ...
      try {
        ...
        const recs = (json?.records || []).map(rec => ({
          ...
          teams: (rec?.teamRecords || []).map(tr => ({
            ...
            abbr: tr?.team?.abbreviation || tr?.team?.teamCode?.toUpperCase?.() || "",
            w: tr?.wins, l: tr?.losses, pct: tr?.winningPercentage, gb: tr?.gamesBack
          }))
        }));
        if (live) setDivisions(recs);
      } catch(e) { if (live) setError(e.message || "Failed to load standings"); }
    ...
    <div style={{ display:"grid", gap:12 }}>
      ...
      {!loading && !error && divisions.map((d, i) => (
        <div key={i} className="card">
          <div className="card-head" style={{ background:"#f8fafc", display:'flex', alignItems:'center' }}>
...
function PlayoffsPanel({ season = CURRENT_SEASON }) {
  ...
  // Helper to parse clinch data from teamRecord variants
  function parseClinchFlags(tr) {
    // MLB StatsAPI variants seen across seasons:
    // - clinchIndicator: one-letter code "x", "y", "z", "w"
    //   x = clinched postseason berth
    //   y = clinched division
    //   z = best league record / homefield
    //   w = clinched wild card (rarely used; included for completeness)
    // - boolean-ish: clinchedDivision, clinchedPostseason, clinchedWildCard, divisionChamp, clinched
    const ci = String(tr?.clinchIndicator || "").toLowerCase();

    const divisionClinched =
      !!(tr?.divisionChamp || tr?.clinchedDivision) ||
      ci === "y" || ci === "z";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const seededGroups = Object.keys(byLg).sort().map(lg => {
          const teams = byLg[lg];
          const leaders = teams
      ...
      } catch(e) { if (live) setError(e.message || "Failed to load playoffs view"); }
      finally { if (live) setLoading(false); }
    })();
    return () => { live = false; };
  }, [season]);
  ...
  return (
    <div style={{ display:"grid", gap:12 }}>
      ...
      {!loading && !error && groups.map((g, i) => (
        <div key={i} className="card">
          <div className="card-head" style={{ background:"#f8fafc", display:'flex', alignItems:'center', justifyContent:"space-between" }}>
            <span style={{ fontWeight:700 }}>{g.league} Playoffs</span>
            <span style={{ fontSize:12, color:"#64748b" }}>Red line = in above (seeds 1–6), out below</span>
          </div>
          <div style={{ overflowX:"auto" }}>
                ...
                {g.teams.map((t) => {
                  ...
                  return (
                    <tr key={t.id} style={rowStyle}>
                      ...
                      <td style={{ padding:8, textAlign:"right" }}>{t.gbPlayoffs == null ? "—" : t.gbPlayoffs.toFixed(1)}</td>
                    </tr>
                  );
                })}
              </tbody>
    ...
    <div style={{ border:"1px solid #e2e8f0", background:"#fff", padding:4, borderRadius:12, display:"inline-flex", gap:4 }}>
      {items.map(it => (
        <button key={it.id} onClick={() => onChange(it.id)}
          className="btn" style={{ background: value===it.id ? "#0f172a" : "#fff", color: value===it.id ? "#fff" : "#0f172a", borderColor:"#e2e8f0" }}>
          {it.label}
        </button>
      ))}
    </div>
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(#f8fafc, #ffffff)" }}>
      <div className="container">
        <header style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          <div><h1 style={{ fontSize:28, margin:0 }}>MLB Daily Dashboard</h1><div style={{ color:"#64748b" }}>Scores, Divisions & Playoffs</div></div>
        ...
        <footer style={{ marginTop:16, fontSize:12, color:"#64748b" }}>Playoffs view seeds 1–6 per league (3 division winners + 3 wild cards). Red line separates in/out.</footer>
      </div>
    </div>
  );
}